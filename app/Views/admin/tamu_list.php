...
<div class="app-toolbar py-3 py-lg-6" id="kt_app_toolbar">
    <div class="app-container container-xxl d-flex flex-stack">
        <div class="page-title d-flex flex-column me-3">
            <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center me-10">Daftar Tamu</h1>
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-5px"></span></li>
                <li class="breadcrumb-item text-gray-500">Daftar Tamu</li>
            </ul>
        </div>
    </div>
</div>
...
<div id="kt_drawer_detail" class="bg-white" data-kt-drawer="true" data-kt-drawer-activate="true" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'350px', 'md': '425px'}" data-kt-drawer-direction="end">
    <div class="card rounded-0 w-100">
        <div class="card-header pe-5" id="kt_drawer_header">
            <h3 class="card-title fw-bold">Detail Tamu</h3>
        ...
        <div class="card-body pt-5" id="kt_drawer_body">
            <!-- Diisi via DOM API -->
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Inisialisasi DataTables dengan column search
    if (typeof $ !== 'undefined' && $.fn.DataTable) {
        table = $('#kt_table_tamu').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
            ...
            // Klik baris untuk buka drawer detail
            createdRow: function (row, data) {
                $(row).css('cursor', 'pointer');
                $(row).on('click', function (e) {
                    // Jangan buka drawer jika klik checkbox, aksi, atau link
    ...
    // === DETAIL DRAWER (menggunakan DOM API yang aman) ===
    function openDetailDrawer(data) {
        ...
        fotoImg.alt = 'foto';
        fotoImg.className = 'symbol-label';
        ...
        info.forEach(function (item) {
            ...
            label.className = 'text-muted fw-semibold fs-7 mb-1';
            label.textContent = item.label;

            var value = document.createElement('div');
            value.className = 'fw-bold text-gray-800 fs-6';
        ...
        editBtn.className = 'btn btn-primary btn-sm fw-bold';
        editBtn.textContent = 'Edit';
        editBtn.addEventListener('click', function () {
            openEditModal(data);
            if (drawerObject) drawerObject.hide();
        });

        var deleteBtn = document.createElement('button');
        deleteBtn.className = 'btn btn-light-danger btn-sm fw-bold';
    ...
    // === MODAL CRUD (tetap dipertahankan dari asli) ===
    ...
    window.openCreateModal = function () {
        ...
        document.getElementById('modal_title').textContent = 'Tambah Tamu';
        currentModal.show();
    };
    ...
    window.openEditModal = function (data) {
        ...
        document.getElementById('modal_title').textContent = 'Edit Tamu';
        currentModal.show();
    };
    ...
    window.submitForm = async function (e) {
        ...
        var url = id ? '<?= base_url('admin/tamu/update/') ?>' + id : '<?= base_url('admin/tamu/store') ?>';
    ...
    window.deleteData = function (id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    var response = await fetch('<?= base_url('admin/tamu/delete/') ?>' + id, {
                        method: 'POST',
                        headers: { 'X-Requested-With': 'XMLHttpRequest' }
                    });
